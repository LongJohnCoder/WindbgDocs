...
function moveTip()
{
	...
	while ((offsetParent.offsetLeft != 0) || (offsetParent.offsetTop != 0)) {
		linkLeft += offsetParent.offsetLeft;
		linkTop += offsetParent.offsetTop;
...
// See Also popups
//
// NOTE:  This was edited by LKing on 2/13/2002.  In the original version
//        it created See Also popups for Rmh's called "See Also".   Now
//        the popups appear for "See Also" and "Additional Information"
//        Rmh's.
//
//        If someone puts BOTH Rmh's (or puts two "See Also"s, etc.) all
//        text under each qualifying Rmh is added to the popup window....
//        but you don't get TWO popup windows.  In other words, this is
//        a style error, but it won't cause a build error.
//
function initSeeAlso()
{
	// Localizable strings.
	var L_See_Also = "See Also";
	var L_Additional_Information = "Additional Information";
	...
	if (heads) {
		for (var i = 0; i < heads.length; i++) {
			...
			if (beginsWith(txt, L_See_Also) || beginsWith(txt, L_Additional_Information)) {
				divS += head.outerHTML;
...
// Glossary Popups
// 
// When the page is loaded, all glossary entries are changed to
// support popups.
// 
// When clicking on a link to a glossary entry, the glossary page is
// loaded into an iframe, then the reftips code is used to display the
// popup.
// ======================================================================

function initGlossPop()
{
  makeGlossPopLinks(false);

}

// Change links to glossary pages to glossary popups.
// Called when the page is loaded with guest=false and when a glossary
// page is loaded with guest=true.
// When guest==true: only change those glossary links that haven't
// been changed and use a different event handler so the popup appears
// in the correct place.
function makeGlossPopLinks (guest) {
  var links = document.links;
  for (var j = 0; j != links.length; j++) {
    var link = links[j];
    var pieces = link.href.split("/");
    var leaf = pieces[pieces.length-1].split("#");
    var page = leaf[0];
    if ((leaf.length == 2)
        && (beginsWith(page, "dbg_glossary"))) {
      link.className = "glosspop";
      if (guest) {
        if (!link.onclick) {
          attach_popup(link, showGuestGlossPop);
        }
      } else {
        attach_popup(link, showGlossPop);
      }
    }
  }
}

// attach closure to element, so that when the element is clicked, the
// function is called passing in the element.
function attach_popup(element, fn) {
  element.onclick = function () {fn(element);};
}

var glosspop_link;
var glosspop_location;
var glosspop_wait;
var iframe_inserted = false;
var iframe_wait;

function showGlossPop(link) {
  if (!iframe_inserted) {
    var iframe = "<iframe style=\"display:none\" id=\"glosspop_iframe\"/>";
    document.body.insertAdjacentHTML('beforeEnd', iframe);
  }
  glosspop_location = link;
  showGuestGlossPop(link);
}

// load glossary page before displaying popup
function showGuestGlossPop(link) {
  glosspop_link = link;
  event.returnValue = false;
  var href = glosspop_link.href.split("#");
  var page = href[0];
  var id = href[1];
  var iframe = window.frames["glosspop_iframe"];
  ...
  iframe.location = page;
  clearInterval(glosspop_wait);
  glosspop_wait = setInterval("is_loaded()", 1);
}

// wait for glossary page to load.
function is_loaded () {
  var iframe = window.frames["glosspop_iframe"];
  if (iframe.document.anchors.length != 0) {
    clearInterval(glosspop_wait);
    glosspage_loaded();
  }
}

// The glossary page is loaded into the iframe.
// Locate the desired entry and display it.
function glosspage_loaded() {
  var href = glosspop_link.href.split("#");
  var id = href[1];
  var iframe = window.frames["glosspop_iframe"];

  var div = "";
  var all = iframe.document.all;
  for (var i = 0; i != all.length; i++) {
    if (all(i).name == id ) {
      div = iframe.document.all(i).parentNode.parentNode.parentNode.outerHTML;
    }
  }
  
  var reftip = document.all.reftip;
  reftip.style.visibility = "hidden";
  reftip.style.pixelWidth = 260;
  reftip.style.pixelHeight = 24;

  reftip.innerHTML = div;
  makeGlossPopLinks(true);
  window.linkElement = glosspop_location;
  window.setTimeout("moveTip()", 0);
  iframe.location = document.location;
}
...
function showall() {
	...
	for(var i = 0; i < imgcount; i++)
		...
		document.all.tags("IMG")[i].src = "down.gif" ;
		}
	...
	for(var i = 0; i < imgcount; i++)
		if(document.all.tags("IMG")[i].className == "Btn") {
		document.all.tags("IMG")[i].src = "right.gif" ;
		}
...
function doSection (secNum, button){
    if (secNum.length > 1){
        if (secNum[0].style.display=="none"){
            secNum[0].style.display = ""        
            button.src = down.src;
        }else{
            secNum[0].style.display = "none"
            button.src = "right.gif"; 
        }
    } else if (secNum.style.display=="none"){
